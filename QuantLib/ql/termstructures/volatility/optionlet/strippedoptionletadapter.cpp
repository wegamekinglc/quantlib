...
StrippedOptionletAdapter::StrippedOptionletAdapter(
    ...
    : OptionletVolatilityStructure(s->settlementDays(), s->calendar(),
                                   s->businessDayConvention(), s->dayCounter()),
      optionletStripper_(s), nInterpolations_(s->optionletMaturities()),
...
StrippedOptionletAdapter::smileSectionImpl(Time t) const {
    std::vector<Rate> optionletStrikes = optionletStripper_->optionletStrikes(
    ...
    // Extrapolation may be a problem with splines, but since minStrike() and
    // maxStrike() are set, we assume that no one will use stddevs for strikes
    // outside these strikes
...
Volatility StrippedOptionletAdapter::volatilityImpl(Time length,
    ...
    boost::shared_ptr<LinearInterpolation> timeInterpolator(
        new LinearInterpolation(optionletTimes.begin(), optionletTimes.end(),
                                vol.begin()));
...
void StrippedOptionletAdapter::performCalculations() const {
    ...
    // const std::vector<Rate>& atmForward =
    // optionletStripper_->atmOptionletRate();
    // const std::vector<Time>& optionletTimes =
    // optionletStripper_->optionletTimes();
    ...
    for (Size i = 0; i < nInterpolations_; ++i) {
        ...
        //            SABRInterpolation(optionletStrikes.begin(),
        ...
        //              ", error " <<
        //              strikeInterpolations_[i]->interpolationError()
        //              );
    }
}
