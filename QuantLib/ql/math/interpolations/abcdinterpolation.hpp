...
class AbcdCoeffHolder {
  public:
    AbcdCoeffHolder(Real a, Real b, Real c, Real d, bool aIsFixed,
                    bool bIsFixed, bool cIsFixed, bool dIsFixed)
        : a_(a), b_(b), c_(c), d_(d), aIsFixed_(false), bIsFixed_(false),
          cIsFixed_(false), dIsFixed_(false), k_(std::vector<Real>()),
...
class AbcdInterpolationImpl : public Interpolation::templateImpl<I1, I2>,
  ...
  public:
    AbcdInterpolationImpl(
        const I1 &xBegin, const I1 &xEnd, const I2 &yBegin, Real a, Real b,
        Real c, Real d, bool aIsFixed, bool bIsFixed, bool cIsFixed,
        bool dIsFixed, bool vegaWeighted,
    ...
    void update() {
        ...
        abcdCalibrator_ =
            boost::shared_ptr<AbcdCalibration>(new AbcdCalibration(
                times, blackVols, a_, b_, c_, d_, aIsFixed_, bIsFixed_,
                cIsFixed_, dIsFixed_, vegaWeighted_, endCriteria_, optMethod_));
    ...
    Real value(Real x) const {
        QL_REQUIRE(x >= 0.0, "time must be non negative: " << x
                                                           << " not allowed");
        return abcdCalibrator_->value(x);
    }
    Real primitive(Real) const { QL_FAIL("Abcd primitive not implemented"); }
    Real derivative(Real) const { QL_FAIL("Abcd derivative not implemented"); }
...
class AbcdInterpolation : public Interpolation {
  public:
    ...
    AbcdInterpolation(const I1 &xBegin, // x = times
                      ...
                      Real a = -0.06, Real b = 0.17, Real c = 0.54,
                      Real d = 0.17, bool aIsFixed = false,
        ...
        impl_ = boost::shared_ptr<Interpolation::Impl>(
            new detail::AbcdInterpolationImpl<I1, I2>(
                xBegin, xEnd, yBegin, a, b, c, d, aIsFixed, bIsFixed, cIsFixed,
                dIsFixed, vegaWeighted, endCriteria, optMethod));
        impl_->update();
        coeffs_ = boost::dynamic_pointer_cast<detail::AbcdCoeffHolder>(impl_);
    }
    //! \name Inspectors
    //@{
...
template <class T = Real> class Abcd {
  public:
    Abcd(Real a, Real b, Real c, Real d, bool aIsFixed, bool bIsFixed,
         bool cIsFixed, bool dIsFixed, bool vegaWeighted = false,
         const boost::shared_ptr<EndCriteria> endCriteria =
             boost::shared_ptr<EndCriteria>(),
         const boost::shared_ptr<OptimizationMethod> optMethod =
             boost::shared_ptr<OptimizationMethod>())
        : a_(a), b_(b), c_(c), d_(d), aIsFixed_(aIsFixed), bIsFixed_(bIsFixed),
          cIsFixed_(cIsFixed), dIsFixed_(dIsFixed), vegaWeighted_(vegaWeighted),
          endCriteria_(endCriteria), optMethod_(optMethod) {}
    template <class I1, class I2>
    Interpolation interpolate(const I1 &xBegin, const I1 &xEnd,
                              const I2 &yBegin) const {
        return AbcdInterpolation(xBegin, xEnd, yBegin, a_, b_, c_, d_,
