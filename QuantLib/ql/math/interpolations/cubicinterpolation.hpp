...
template <class T> class CoefficientHolder_t {
  public:
    CoefficientHolder_t(Size n)
        : n_(n), primitiveConst_(n - 1), a_(n - 1), b_(n - 1), c_(n - 1),
          monotonicityAdjustments_(n) {}
    virtual ~CoefficientHolder_t() {}
    Size n_;
    // P[i](x) = y[i] +
    //           a[i]*(x-x[i]) +
    //           b[i]*(x-x[i])^2 +
    //           c[i]*(x-x[i])^3
    std::vector<T> primitiveConst_, a_, b_, c_;
    std::vector<bool> monotonicityAdjustments_;
};

template <class I1, class I2, class T> class CubicInterpolationImpl_t;
}
...
template <class T> class CubicInterpolation_t : public Interpolation_t<T> {
  public:
    ...
    CubicInterpolation_t(const I1 &xBegin, const I1 &xEnd, const I2 &yBegin,
                         CubicInterpolation_t<T>::DerivativeApprox da,
                         bool monotonic,
                         CubicInterpolation_t<T>::BoundaryCondition leftCond,
        ...
        this->impl_->update();
        coeffs_ = boost::dynamic_pointer_cast<detail::CoefficientHolder_t<T> >(
            this->impl_);
    }
    const std::vector<T> &primitiveConstants() const {
        return coeffs_->primitiveConst_;
    }
    const std::vector<T> &aCoefficients() const { return coeffs_->a_; }
    const std::vector<T> &bCoefficients() const { return coeffs_->b_; }
    const std::vector<T> &cCoefficients() const { return coeffs_->c_; }
    const std::vector<bool> &monotonicityAdjustments() const {
        return coeffs_->monotonicityAdjustments_;
    }

  private:
    boost::shared_ptr<detail::CoefficientHolder_t<T> > coeffs_;
};
...
template <class T> class CubicNaturalSpline_t : public CubicInterpolation_t<T> {
  public:
    /*! \pre the \f$ x \f$ values must be sorted. */
    template <class I1, class I2>
    CubicNaturalSpline_t(const I1 &xBegin, const I1 &xEnd, const I2 &yBegin)
        : CubicInterpolation_t<T>(xBegin, xEnd, yBegin, this->Spline, false,
                                  this->SecondDerivative, 0.0,
                                  this->SecondDerivative, 0.0) {}
};

typedef CubicNaturalSpline_t<Real> CubicNaturalSpline;

template <class T>
class MonotonicCubicNaturalSpline_t : public CubicInterpolation_t<T> {
  public:
    /*! \pre the \f$ x \f$ values must be sorted. */
    template <class I1, class I2>
    MonotonicCubicNaturalSpline_t(const I1 &xBegin, const I1 &xEnd,
                                  const I2 &yBegin)
        : CubicInterpolation_t<T>(xBegin, xEnd, yBegin, this->Spline, true,
                                  this->SecondDerivative, 0.0,
                                  this->SecondDerivative, 0.0) {}
};

typedef MonotonicCubicNaturalSpline_t<Real> MonotonicCubicNaturalSpline;

template <class T>
class CubicSplineOvershootingMinimization1_t : public CubicInterpolation_t<T> {
  public:
    /*! \pre the \f$ x \f$ values must be sorted. */
    template <class I1, class I2>
    CubicSplineOvershootingMinimization1_t(const I1 &xBegin, const I1 &xEnd,
                                           const I2 &yBegin)
        : CubicInterpolation_t<T>(xBegin, xEnd, yBegin, this->SplineOM1, false,
                                  this->SecondDerivative, 0.0,
                                  this->SecondDerivative, 0.0) {}
};

typedef CubicSplineOvershootingMinimization1_t<Real>
    CubicSplineOvershootingMinimization1;

template <class T>
class CubicSplineOvershootingMinimization2_t : public CubicInterpolation_t<T> {
  public:
    /*! \pre the \f$ x \f$ values must be sorted. */
    template <class I1, class I2>
    CubicSplineOvershootingMinimization2_t(const I1 &xBegin, const I1 &xEnd,
                                           const I2 &yBegin)
        : CubicInterpolation_t<T>(xBegin, xEnd, yBegin, this->SplineOM2, false,
                                  this->SecondDerivative, 0.0,
                                  this->SecondDerivative, 0.0) {}
};

typedef CubicSplineOvershootingMinimization2_t<Real>
    CubicSplineOvershootingMinimization2;

template <class T>
class AkimaCubicInterpolation_t : public CubicInterpolation_t<T> {
  public:
    /*! \pre the \f$ x \f$ values must be sorted. */
    template <class I1, class I2>
    AkimaCubicInterpolation_t(const I1 &xBegin, const I1 &xEnd,
                              const I2 &yBegin)
        : CubicInterpolation_t<T>(xBegin, xEnd, yBegin, this->Akima, false,
                                  this->SecondDerivative, 0.0,
                                  this->SecondDerivative, 0.0) {}
};

typedef AkimaCubicInterpolation_t<Real> AkimaCubicInterpolation;

template <class T> class KrugerCubic_t : public CubicInterpolation_t<T> {
  public:
    /*! \pre the \f$ x \f$ values must be sorted. */
    template <class I1, class I2>
    KrugerCubic_t(const I1 &xBegin, const I1 &xEnd, const I2 &yBegin)
        : CubicInterpolation_t<T>(xBegin, xEnd, yBegin, this->Kruger, false,
                                  this->SecondDerivative, 0.0,
                                  this->SecondDerivative, 0.0) {}
};

typedef KrugerCubic_t<Real> KrugerCubic;

template <class T>
class FritschButlandCubic_t : public CubicInterpolation_t<T> {
  public:
    /*! \pre the \f$ x \f$ values must be sorted. */
    template <class I1, class I2>
    FritschButlandCubic_t(const I1 &xBegin, const I1 &xEnd, const I2 &yBegin)
        : CubicInterpolation_t<T>(xBegin, xEnd, yBegin, this->FritschButland,
                                  false, this->SecondDerivative, 0.0,
                                  this->SecondDerivative, 0.0) {}
};

typedef FritschButlandCubic_t<Real> FritschButlandCubic;

template <class T> class Parabolic_t : public CubicInterpolation_t<T> {
  public:
    /*! \pre the \f$ x \f$ values must be sorted. */
    template <class I1, class I2>
    Parabolic_t(const I1 &xBegin, const I1 &xEnd, const I2 &yBegin)
        : CubicInterpolation_t<T>(
              xBegin, xEnd, yBegin, CubicInterpolation_t<T>::Parabolic, false,
              this->SecondDerivative, 0.0, this->SecondDerivative, 0.0) {}
};

typedef Parabolic_t<Real> Parabolic;

template <class T> class MonotonicParabolic_t : public CubicInterpolation_t<T> {
  public:
    /*! \pre the \f$ x \f$ values must be sorted. */
    template <class I1, class I2>
    MonotonicParabolic_t(const I1 &xBegin, const I1 &xEnd, const I2 &yBegin)
        : CubicInterpolation_t<T>(xBegin, xEnd, yBegin, this->Parabolic, true,
                                  this->SecondDerivative, 0.0,
                                  this->SecondDerivative, 0.0) {}
};

typedef MonotonicParabolic_t<Real> MonotonicParabolic;

//! %Cubic interpolation factory and traits
template <class T = Real> class Cubic {
  public:
    Cubic(typename CubicInterpolation_t<T>::DerivativeApprox da =
              CubicInterpolation_t<T>::Kruger,
          bool monotonic = false,
          typename CubicInterpolation_t<T>::BoundaryCondition leftCondition =
              CubicInterpolation_t<T>::SecondDerivative,
          T leftConditionValue = 0.0,
          typename CubicInterpolation_t<T>::BoundaryCondition rightCondition =
              CubicInterpolation_t<T>::SecondDerivative,
          T rightConditionValue = 0.0)
    ...
    Interpolation_t<T> interpolate(const I1 &xBegin, const I1 &xEnd,
        ...
        return CubicInterpolation_t<T>(xBegin, xEnd, yBegin, da_, monotonic_,
                                       leftType_, leftValue_, rightType_,
                                       rightValue_);
    }
  ...
  private:
    typename CubicInterpolation_t<T>::DerivativeApprox da_;
    bool monotonic_;
    typename CubicInterpolation_t<T>::BoundaryCondition leftType_, rightType_;
...
class CubicInterpolationImpl_t : public CoefficientHolder_t<T>,
  ...
  public:
    CubicInterpolationImpl_t(
        ...
        typename CubicInterpolation_t<T>::DerivativeApprox da, bool monotonic,
        typename CubicInterpolation_t<T>::BoundaryCondition leftCondition,
        T leftConditionValue,
        typename CubicInterpolation_t<T>::BoundaryCondition rightCondition,
        T rightConditionValue)
        ...
        if (leftType_ == CubicInterpolation_t<T>::Lagrange ||
            rightType_ == CubicInterpolation_t<T>::Lagrange) {
            QL_REQUIRE((xEnd - xBegin) >= 4,
                       "Lagrange boundary condition requires at least "
                       "4 points ("
                           << (xEnd - xBegin) << " are given)");
        }
    }
    ...
    void update() {
        ...
        if (da_ == CubicInterpolation_t<T>::Spline) {
            for (Size i = 1; i < this->n_ - 1; ++i) {
                L_.setMidRow(i, dx_[i], 2.0 * (dx_[i] + dx_[i - 1]),
                             dx_[i - 1]);
            ...
            switch (leftType_) {
            case CubicInterpolation_t<T>::NotAKnot:
            ...
            case CubicInterpolation_t<T>::Lagrange:
                L_.setFirstRow(1.0, 0.0);
                tmp_[0] = cubicInterpolatingPolynomialDerivative(
                    this->xBegin_[0], this->xBegin_[1], this->xBegin_[2],
                    this->xBegin_[3], this->yBegin_[0], this->yBegin_[1],
                    this->yBegin_[2], this->yBegin_[3], this->xBegin_[0]);
                break;
            default:
            ...
            case CubicInterpolation_t<T>::Lagrange:
                L_.setLastRow(0.0, 1.0);
                tmp_[this->n_ - 1] = cubicInterpolatingPolynomialDerivative(
                    this->xBegin_[this->n_ - 4], this->xBegin_[this->n_ - 3],
                    this->xBegin_[this->n_ - 2], this->xBegin_[this->n_ - 1],
                    this->yBegin_[this->n_ - 4], this->yBegin_[this->n_ - 3],
                    this->yBegin_[this->n_ - 2], this->yBegin_[this->n_ - 1],
                    this->xBegin_[this->n_ - 1]);
                break;
            default:
                QL_FAIL("unknown end condition");
            }
        ...
        } else if (da_ == CubicInterpolation_t<T>::SplineOM1) {
            Matrix_t<T> T_(this->n_ - 2, this->n_, 0.0);
            for (Size i = 0; i < this->n_ - 2; ++i) {
                T_[i][i] = dx_[i] / 6.0;
                T_[i][i + 1] = (dx_[i + 1] + dx_[i]) / 3.0;
                T_[i][i + 2] = dx_[i + 1] / 6.0;
            }
            Matrix_t<T> S_(this->n_ - 2, this->n_, 0.0);
            for (Size i = 0; i < this->n_ - 2; ++i) {
                S_[i][i] = 1.0 / dx_[i];
                S_[i][i + 1] = -(1.0 / dx_[i + 1] + 1.0 / dx_[i]);
                S_[i][i + 2] = 1.0 / dx_[i + 1];
            }
            Matrix_t<T> Up_(this->n_, 2, 0.0);
            Up_[0][0] = 1;
            Up_[this->n_ - 1][1] = 1;
            Matrix_t<T> Us_(this->n_, this->n_ - 2, 0.0);
            for (Size i = 0; i < this->n_ - 2; ++i)
                Us_[i + 1][i] = 1;
            Matrix_t<T> Z_ = Us_ * inverse(T_ * Us_);
            Matrix_t<T> I_(this->n_, this->n_, 0.0);
            for (Size i = 0; i < this->n_; ++i)
                I_[i][i] = 1;
            Matrix_t<T> V_ = (I_ - Z_ * T_) * Up_;
            Matrix_t<T> W_ = Z_ * S_;
            ...
            for (Size i = 1; i < this->n_ - 1; ++i) {
                Q_[i][i - 1] = 7.0 / 8 * 1.0 / (this->n_ - 1) * dx_[i - 1] *
                               dx_[i - 1] * dx_[i - 1];
                Q_[i][i] =
                    1.0 / (this->n_ - 1) * dx_[i] * dx_[i] * dx_[i] +
                    1.0 / (this->n_ - 1) * dx_[i - 1] * dx_[i - 1] * dx_[i - 1];
                Q_[i][i + 1] =
            ...
            if (this->n_ == 2)
                ...
                switch (da_) {
                case CubicInterpolation_t<T>::FourthOrder:
                    QL_FAIL("FourthOrder not implemented yet");
                    break;
                case CubicInterpolation_t<T>::Parabolic:
                    // intermediate points
                    for (Size i = 1; i < this->n_ - 1; ++i)
                        tmp_[i] = (dx_[i - 1] * S_[i] + dx_[i] * S_[i - 1]) /
                                  (dx_[i] + dx_[i - 1]);
                    // end points
                    tmp_[0] =
                ...
                case CubicInterpolation_t<T>::Akima:
                    ...
                    for (Size i = 2; i < this->n_ - 2; ++i) {
                        if ((S_[i - 2] == S_[i - 1]) && (S_[i] != S_[i + 1]))
                            tmp_[i] = S_[i - 1];
                        else if ((S_[i - 2] != S_[i - 1]) &&
                                 (S_[i] == S_[i + 1]))
                            tmp_[i] = S_[i];
                        else if (S_[i] == S_[i - 1])
                            tmp_[i] = S_[i];
                        else if ((S_[i - 2] == S_[i - 1]) &&
                                 (S_[i - 1] != S_[i]) && (S_[i] == S_[i + 1]))
        ...
        std::fill(this->monotonicityAdjustments_.begin(),
                  this->monotonicityAdjustments_.end(), false);
            ...
            for (Size i = 0; i < this->n_; ++i) {
                if (i == 0) {
                    ...
                    if (correction != tmp_[i]) {
                        tmp_[i] = correction;
                        this->monotonicityAdjustments_[i] = true;
                    }
                } else if (i == this->n_ - 1) {
        ...
        for (Size i = 1; i < this->n_ - 1; ++i) {
            this->primitiveConst_[i] =
                this->primitiveConst_[i - 1] +
                dx_[i - 1] *
                    (this->yBegin_[i - 1] +
                     dx_[i - 1] *
                         (this->a_[i - 1] / 2.0 +
                          dx_[i - 1] * (this->b_[i - 1] / 3.0 +
                                        dx_[i - 1] * this->c_[i - 1] / 4.0)));
        }
    }
    T value(T x) const {
  ...
  private:
    ...
    inline T cubicInterpolatingPolynomialDerivative(T a, T b, T c, T d, T u,
        ...
        return (-((((a - c) * (b - c) * (c - x) * z -
                    (a - d) * (b - d) * (d - x) * w) *
                       (a - x + b - x) +
                   ((a - c) * (b - c) * z - (a - d) * (b - d) * w) * (a - x) *
                       (b - x)) *
                      (a - b) +
                  ((a - c) * (a - d) * v - (b - c) * (b - d) * u) * (c - d) *
                      (c - x) * (d - x) +
                  ((a - c) * (a - d) * (a - x) * v -
                   (b - c) * (b - d) * (b - x) * u) *
                      (c - x + d - x) * (c - d))) /
