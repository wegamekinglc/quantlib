...
*/
template <class T> class Instrument_t : public LazyObject {
  public:
    class results;
    Instrument_t();
    //! \name Inspectors
...
inline Instrument_t<T>::Instrument_t()
    : NPV_(Null<Real>()), errorEstimate_(Null<Real>()), valuationDate_(Date()) {
}

template <class T>
inline void
Instrument_t<T>::setPricingEngine(const boost::shared_ptr<PricingEngine> &e) {
    if (engine_)
...
template <class T> inline void Instrument_t<T>::calculate() const {
    if (isExpired()) {
        setupExpired();
        calculated_ = true;
    } else {
...
template <class T> inline void Instrument_t<T>::setupExpired() const {
    NPV_ = errorEstimate_ = 0.0;
    valuationDate_ = Date();
    additionalResults_.clear();
}

template <class T> inline void Instrument_t<T>::performCalculations() const {
...
template <class T>
inline void
Instrument_t<T>::fetchResults(const PricingEngine::results *r) const {
...
template <class T> inline T Instrument_t<T>::NPV() const {
    calculate();
    QL_REQUIRE(NPV_ != Null<Real>(), "NPV not provided");
    return NPV_;
}

template <class T> inline T Instrument_t<T>::errorEstimate() const {
    calculate();
    QL_REQUIRE(errorEstimate_ != Null<Real>(), "error estimate not provided");
    return errorEstimate_;
}

template <class T> inline const Date &Instrument_t<T>::valuationDate() const {
    calculate();
